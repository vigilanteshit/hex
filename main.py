...
if __name__ == '__main__':
    ...
    brevity_encouragment = [0.001] # small positive number encourages shorter games, a negative number encourages longer games
    similarity_panelty_list = [0.001] # small negative number encourages vertical exploration, positive number encourages horizontal moves
    move_similarity_penalty_list = [-0.01] # small negative number discourages playing similar moves in consecutive games
    ...
    gradient_clipping_parameter_list = [0.4]
    number_of_eval_games_list = [20] # the number of games for evaluation
    training_loop_list = [training_loop_configs.loop_code_baseline1,   
                         training_loop_configs.loop_code_baseline2,
                         training_loop_configs.loop_code_baseline3,
                         training_loop_configs.loop_code_baseline4]
    policy_net_list = [PolicyNet.create_complex_hex_policy_net(board_size=BOARD_SIZE), 
                       PolicyNet.create_dense_nn_policy_net(board_size=BOARD_SIZE)]
    


    # Give training ID for the job
    training_run_base_id = 'Test_run'
